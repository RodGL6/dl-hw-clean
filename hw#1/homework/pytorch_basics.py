...
class PyTorchBasics:
    ...
    def make_it_pytorch_10(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        # Alternative
        # return torch.where(x < c, torch.tensor(0.0, device=x.device), x)
        # or return torch.maximum(x, torch.zeros_like(x)) * (x >= c)
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.sum((torch.abs(x.unsqueeze(1) - y) < 1e-3).any(dim=1))


        # Alternative:
        #   differences = torch.abs(x[:, None] - y[None, :])  # Pairwise differences using broadcasting
        #   matches = differences < 1e-3  # Boolean mask for matches
        #   return torch.sum(matches.any(dim=1)) # Check if each x_i has a match in y, then count them
